...
hangbot token path = do
  ...
  runSlackbot token mempty (userContextBot hangman)
...
hangman :: Slackbot (Maybe Hangman)
hangman = Slackbot f
  where
    f MessageContext{..} Nothing _ = ([OutgoingMessage 1 "message" mcChannel "Start guessing!"], Just $ mkHangman "word")
    f MessageContext{..} (Just game) m
      | Just c <- findChar m =
              let ctx = guess c game
                  out m = OutgoingMessage 1 "message" mcChannel m
              in ([out (T.pack . partialSolution $ ctx)], Just ctx)
      | otherwise = ([], Just game)
